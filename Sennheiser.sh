...
pactl list sink-inputs | grep -oP '(?<=Sink Input #|application.process.binary = "|Sink: )[^"]*' | while read STREAMID; do
 read OLDSINK
 read APPLICATION
 ...
 if [ "$APPLICATION" == "$FILTER_APPLICATION" ]; then
  echo "switching stream $STREAMID($APPLICATION) to sink $SINK(now $DEFAULT_SINKEXPRESSION was $OLDSINK)"
